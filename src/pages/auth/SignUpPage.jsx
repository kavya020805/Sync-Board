  ...
  return (
    <div className="w-full max-w-[400px] animate-fade-in">
      ...
      <div className="flex items-center gap-2.5 mb-10 lg:hidden">
        ...
        <span className="font-bold text-xl text-(--color-text-primary) tracking-tight">Sync Board</span>
      </div>

      <div className="mb-8">
        <h1 className="text-2xl font-bold text-(--color-text-primary) tracking-tight mb-2">Create an account</h1>
        <p className="text-sm text-(--color-text-secondary)">Join Sync Board and start collaborating</p>
      </div>
      ...
      <div className="flex flex-col gap-3 mb-6">
        <button
          ...
          className="flex items-center justify-center gap-3 w-full h-10 rounded-md border border-(--color-border-default) bg-(--color-bg-primary) text-sm font-medium text-(--color-text-primary) transition-all hover:bg-(--color-bg-hover) hover:border-(--color-border-strong) cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {oauthLoading === 'google' ? (
        ...
        >
          {oauthLoading === 'github' ? (
            ...
            <svg className="w-4 h-4 text-(--color-text-primary)" fill="currentColor" viewBox="0 0 24 24">
